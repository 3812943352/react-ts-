 ...
 * @FilePath: src/views/Page10/index.tsx
 * @Description: 请填写简介
 */
...
import { getAllUser, updateUser, userBlurApi } from "@/api/user.ts";
import { authOptions } from "@/views/Page10/options/authOptins.ts";
import { formatDate } from "@/commponents/formatDate.ts";
import { updateUserByIdDataType } from "@/types/user.ts";
...
const View: React.FC = () => {
  ...
  const save = async (record: any) => {
    ...
    const req: updateUserByIdDataType = {
      data: {
        id: record.id,
        ...row,
      },
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "phone",
      width: "5%",
      sorter: (a: { phone: any }, b: { phone: any }) =>
    ...
    },
    {
      ...
      cascader: true,
      editable: true,
      sorter: (a: { auth: any }, b: { auth: any }) =>
        a.auth.length - b.auth.length,
      sortDirections: ["descend", "ascend"],
      ellipsis: {
        showTitle: false,
      },
    ...
    },
    {
      ...
      width: "5%",
      sorter: (a: any, b: any) =>
        a.lastLogin.length - b.lastLogin.length,
      sortDirections: ["descend", "ascend"],
      ellipsis: {
        showTitle: false,
      },
  ...
  return (
    <Card hoverable={true} style={{ cursor: "default" }}>
      <Space style={{ marginBottom: 16 }}>
        <Tooltip placement="topLeft" title="模糊查询手机号">
          <Search
